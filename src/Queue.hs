...
set i x (Queue es r) =
    if inRange i r
        then Queue (es // [(i, x)]) r
        else error ("Out of bound set in Queue at index: " ++ (show i))
...
get i (Queue es r) =
    if inRange i r
        then es ! i
        else error ("Out of bounds get in Queue at index: " ++ (show i))
...
find (SubNewToOld s) cond q =
    if inRange s (range q)
        ...
        else error ("Out of bounds search sub new to old in Queue at index: " ++ (show s))
